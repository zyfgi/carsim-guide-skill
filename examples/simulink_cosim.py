...
def main():
    ...
    ap.add_argument("--matlab", default=None, help="path to matlab.exe "
                    "(default: cached by setup_paths.py)")
    ...
    sim_path = cb.make_scenario(
        ...
        speed_rows=[(0, 50), (args.tstop, 50)],
        steer_rows=[(0, 0), (args.tstop, 0)], unsafe_extra_lines=EXTRA_LINES)
    ...
    cmd = [args.matlab, "-batch",
           "addpath('%s'); addpath('%s'); cosim_model('%s', '%s', %.17g, %.17g)"
